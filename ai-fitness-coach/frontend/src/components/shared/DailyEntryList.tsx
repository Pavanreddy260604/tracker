...
interface Entry {
  _id: string;
  // backend field is 'description'; older logs may have 'name' as fallback
  description?: string;
  name?: string;
  calories: number;
  protein: number;
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      <div className="space-y-3">
        {entries.length > 0 ? (
          entries.map((entry) => {
            const displayName = entry.description || entry.name || 'Food Item';
            const servingsLabel = entry.servings && entry.servings !== 1
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-4 min-w-0">
                  <div className="h-10 w-10 rounded-xl bg-primary/5 text-primary flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex items-center space-x-4 flex-shrink-0">
                  <div className="text-right">
                    <p className="text-sm font-black text-foreground">
                      {entry.calories} <span className="text-[10px] font-normal text-muted-foreground">kcal</span>
                    </p>

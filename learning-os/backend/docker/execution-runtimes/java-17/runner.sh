...
# Decode base64 code
echo "$CODE" | base64 -d > /tmp/Solution.java
...
if [ -f /tmp/input.txt ]; then
    timeout "$TIMEOUT_SEC" java Solution < /tmp/input.txt > /tmp/output.txt 2> /tmp/error.txt
else
    timeout "$TIMEOUT_SEC" java Solution > /tmp/output.txt 2> /tmp/error.txt
fi

...
# Decode base64 code
echo "$CODE" | base64 -d > /tmp/solution.go
...
# Run with timeout (Go compiles and runs in one step)
if [ -f /tmp/input.txt ]; then
    timeout "$TIMEOUT_SEC" go run /tmp/solution.go < /tmp/input.txt > /tmp/output.txt 2> /tmp/error.txt
else
    timeout "$TIMEOUT_SEC" go run /tmp/solution.go > /tmp/output.txt 2> /tmp/error.txt
fi

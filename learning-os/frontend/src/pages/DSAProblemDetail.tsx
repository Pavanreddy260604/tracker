        ...
        <div className="max-w-4xl mx-auto space-y-6 pb-20">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    <h1 className="text-2xl font-bold text-white">{problem.problemName}</h1>
                </div>
            </div>
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex items-center justify-between bg-white/5 p-3 rounded-lg">
                                <span className="capitalize text-white font-medium">{problem.platform}</span>
                        ...
                        {problem.timeSpent > 0 && (
                            <div className="flex items-center gap-2 text-gray-400 text-sm">
                                <Clock size={16} />
                                <span>Time Spent: <span className="text-white font-mono">{problem.timeSpent}m</span></span>
                            </div>
                        )}

                        {problem.companyTags && problem.companyTags.length > 0 && (
                            <div className="flex flex-col gap-2">
                                <label className="text-xs text-gray-500 uppercase tracking-wide">Companies</label>
                                <div className="flex flex-wrap gap-2">
                                    {problem.companyTags.map(tag => (
                                        <span key={tag} className="flex items-center gap-1 text-xs bg-gray-800 text-gray-300 px-2 py-1 rounded">
                                            <Building2 size={12} /> {tag}
                                        </span>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            {problem.timeComplexity && (
                                <div className="flex justify-between items-center text-sm bg-white/5 px-3 py-2 rounded">
                                    <span className="text-gray-400 flex items-center gap-2"><Cpu size={14} /> Time</span>
                                    <span className="font-mono text-amber-400">{problem.timeComplexity}</span>
                                </div>
                            )}
                            {problem.spaceComplexity && (
                                <div className="flex justify-between items-center text-sm bg-white/5 px-3 py-2 rounded">
                                    <span className="text-gray-400 flex items-center gap-2"><Cpu size={14} /> Space</span>
                                    <span className="font-mono text-blue-400">{problem.spaceComplexity}</span>
                                </div>
                            )}
                        </div>
                    </motion.div>
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    >
                        <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                            <BrainCircuit className="text-purple-400" /> Pattern & Approach
                        </h3>
                        {problem.patternLearned ? (
                            <div className="prose prose-invert max-w-none">
                                <p className="text-gray-300 whitespace-pre-wrap text-lg leading-relaxed">{problem.patternLearned}</p>
                            </div>
                        ) : (
                            <p className="text-gray-500 italic">No pattern notes added.</p>
                        )}
                    </motion.div>
                    ...
                    {problem.solutionCode && (
                        <motion.div
                            ...
                            className="bg-[#1c2128] border border-white/10 rounded-xl overflow-hidden"
                        >
                            <div className="bg-white/5 px-4 py-3 border-b border-white/10 flex items-center gap-2">
                                <Code2 size={18} className="text-blue-400" />
                                <span className="font-semibold text-gray-200">Solution Code</span>
                            </div>
                    ...
                    {problem.mistakes && (
                        ...
                        >
                            ...
                            <p className="text-gray-300">{problem.mistakes}</p>
                        </motion.div>
                    )}
